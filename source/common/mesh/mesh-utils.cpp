...
#define WHITE   Project::Color(255, 255, 255, 255)
#define GRAY    Project::Color(128, 128, 128, 255)
#define BLACK   Project::Color(  0,   0,   0, 255)
#define RED     Project::Color(255,   0,   0, 255)
#define GREEN   Project::Color(  0, 255,   0, 255)
#define BLUE    Project::Color(  0,   0, 255, 255)
#define MAGENTA Project::Color(255,   0, 255, 255)
#define YELLOW  Project::Color(255, 255,   0, 255)
#define CYAN    Project::Color(  0, 255, 255, 255)

bool Project::mesh_utils::loadOBJ(Project::Mesh &mesh, const char* filename) {
    ...
    // The data that we will use to initialize Project mesh
    std::vector<Project::Vertex> vertices;
    ...
    std::unordered_map<Project::Vertex, GLuint> vertex_map;
    ...
    mesh.create({Project::setup_buffer_accessors<Vertex>});
...
void Project::mesh_utils::Cuboid(Mesh& mesh,
            bool colored_faces,
    ...
    std::vector<Vertex> vertices = {
            //Upper Face
            {corners[2], colored_faces ? GREEN : GREEN , tex_coords[0], normals[1][1]},
            {corners[3], colored_faces ? GREEN : GREEN, tex_coords[2], normals[1][1]},
            {corners[7], colored_faces ? GREEN : GREEN, tex_coords[3], normals[1][1]},
            {corners[6], colored_faces ? GREEN : GREEN, tex_coords[1], normals[1][1]},
            //Lower Face
            {corners[0], colored_faces ? MAGENTA : GREEN, tex_coords[0], normals[1][0]},
            {corners[4], colored_faces ? MAGENTA : GREEN, tex_coords[2], normals[1][0]},
            {corners[5], colored_faces ? MAGENTA : GREEN, tex_coords[3], normals[1][0]},
            {corners[1], colored_faces ? MAGENTA : GREEN, tex_coords[1], normals[1][0]},
            //Right Face
            {corners[4], colored_faces ? RED : GREEN, tex_coords[0], normals[0][1]},
            {corners[6], colored_faces ? RED : GREEN, tex_coords[2], normals[0][1]},
            {corners[7], colored_faces ? RED : GREEN, tex_coords[3], normals[0][1]},
            {corners[5], colored_faces ? RED : GREEN, tex_coords[1], normals[0][1]},
            //Left Face
            {corners[0], colored_faces ? CYAN : GREEN, tex_coords[0], normals[0][0]},
            {corners[1], colored_faces ? CYAN : GREEN, tex_coords[2], normals[0][0]},
            {corners[3], colored_faces ? CYAN : GREEN, tex_coords[3], normals[0][0]},
            {corners[2], colored_faces ? CYAN : GREEN, tex_coords[1], normals[0][0]},
            //Front Face
            {corners[1], colored_faces ? BLUE : GREEN, tex_coords[0], normals[2][1]},
            {corners[5], colored_faces ? BLUE : GREEN, tex_coords[2], normals[2][1]},
            {corners[7], colored_faces ? BLUE : GREEN, tex_coords[3], normals[2][1]},
            {corners[3], colored_faces ? BLUE : GREEN, tex_coords[1], normals[2][1]},
            //Back Face
            {corners[0], colored_faces ? YELLOW : GREEN, tex_coords[0], normals[2][0]},
            {corners[2], colored_faces ? YELLOW : GREEN, tex_coords[2], normals[2][0]},
            {corners[6], colored_faces ? YELLOW : GREEN, tex_coords[3], normals[2][0]},
            {corners[4], colored_faces ? YELLOW : GREEN, tex_coords[1], normals[2][0]},
    };
...
void Project::mesh_utils::Sphere(Project::Mesh& mesh, const glm::ivec2& segments, bool colored,
    ...
    for(int lat = 0; lat <= segments.y; lat++){
        ...
        for(int lng = 0; lng <= segments.x; lng++){
            ...
            Project::Color color = colored ? Project::Color(127.5f * (normal + 1.0f), 255) : WHITE;
...
void Project::mesh_utils::Plane(Project::Mesh& mesh, const glm::ivec2& resolution, bool colored,
    ...
    for(it.x = 0; it.x <= resolution.x; it.x++){
        ...
        for(it.y = 0; it.y <= resolution.y; it.y++){
            ...
            Project::Color color = colored ? glm::mix(
                    glm::mix(Project::Color(255, 0, 0, 255), Project::Color(0, 255, 0, 255), uv.s),
                    glm::mix(Project::Color(255, 255, 0, 255), Project::Color(0, 0, 255, 255), uv.s),
                    uv.t) : WHITE;
