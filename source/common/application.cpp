...
void Project::Application::configureOpenGL() {
...
Project::WindowConfiguration Project::Application::getWindowConfiguration() {
...
int Project::Application::run() {
    ...
    glfwMakeContextCurrent(window);         // Tell GLFW to make the context of Project window the main context on the current thread.
    ...
    while(!glfwWindowShouldClose(window)){
        ...
        // If ImGui is using the mouse or keyboard, then we don't want the captured events to affect Project keyboard and mouse objects.
        ...
        if(keyboard.justPressed(GLFW_KEY_F12)){
            ...
            if(Project::screenshot_png(stream.str())){
...
// Sets-up the window callback functions from GLFW to Project (Mouse/Keyboard) classes.
void Project::Application::setupCallbacks() {
    ...
    // In the inline function we retrieve the window instance and use it to set Project (Mouse/Keyboard) classes values.
